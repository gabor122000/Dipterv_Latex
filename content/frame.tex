...
A rendszer telepítése és üzemeltetése Kubernetes klaszterben történik.
Ez a környezet garantálja, hogy a Python vezérlő,
a végpontok, valamint a Prometheus és Grafana szolgáltatások
folyamatosan elérhetők legyenek.

\subsubsection{Control Server (Vezérlő)}

A központi logikát tartalmazó Python alapú vezérlő Kubernetes \texttt{Deployment}
formájában jön létre, amelyet egy \texttt{Service} tesz elérhetővé a klaszteren belül.
A kommunikáció biztonságoságát a \texttt{Deployment} manifestjében definiált,
TLS tanúsítványokat tartalmazó \texttt{Secret} biztosítja.

A Flask alapú REST API két fő végpontot kínál:
\begin{itemize}
    \item \texttt{/metrics}: Ez prometheus kompatibilis formátumban mutatja
    az aktuális rendszerállapotot és fogyasztási adatokat.
    \item Vezérlő végpontok: Itt fogadja a rendszer a beavatkozásra adott parancsokat.
\end{itemize}

\subsubsection{ESP8266 Szenzor}

Az ESP8266 szenzorok és a rendszer közötti kapcsolatot
mikroszolgáltatások biztosítják.
Ezeket a Kubernetes Podokat HTTPS-en keresztül látja a vezérlő,
ezeknek folyamatosan figyeli a \texttt{/metrics} végpontját.

Az elvárt "Plug-and-Play" működést a Kubernetes annotációi teszik lehetővé.
Ha egy új eszköz kerül telepítésre  akkor ennek a manifest tartalmazza a
\texttt{prometheus.io/scrape: "true"} és \texttt{prometheus.io/path: "/metrics"}
értékeit, a Prometheus, pedig automatikusan,
új konfiguráció beállítása nélkül is fel fogja venni ezt az eszközt.

\subsubsection{Adattárolás és Vizualizáció}

Az adatbiztonság miatt a Prometheus nem egyszerű Deployment,
hanem \texttt{StatefulSet}.
Az adatbázist egy \texttt{PersistentVolumeClaim} segítségével
tartós tárhelyre menti, így egy újraindítás után is megmaradnak az adatok.
A scrape config-ot, \texttt{ConfigMap}-ekben lehet definiálni.

A Grafana vizualizációs felület is tartós tárhelyet használ
a dashboard-ok és felhasználói beállítások mentésére.
...
\subsection{Helyivezérlő és Szerver között (Wi-Fi)}
...
\subsection{Energetikai eszköz és helyivezrlő között (Modbus)}
