...
A \emph{scenario} JSON séma tömören:
\begin{verbatim}
{
  "name": "My Demo",
  "thresholds": {
...
\begin{enumerate}
  ...
  \item Opcionális \texttt{output.txt} törlés, SIM idők \emph{reset}.
  \item \texttt{thresholds.txt}, \texttt{esp\{1..3\}\_schedule.txt} atomikus írása.
  \item Végállapot: \texttt{RUNNING} vagy \texttt{STOPPED} (opciótól függően).
\end{enumerate}

\subsection{UI szervezés (\emph{Scenarios} kártya)}
\paragraph{Presets.} Előre definiált demók (smoke test, under budget, fair split 3/3/3, 
dinamikus újraelosztás, hiszterézis, STOPPED invariánsok) futtatása opciókkal: 
\emph{Reset t=0}, \emph{Clear output.txt}, \emph{Start after load}. Külső JSON is betölthető/futtatható.

\paragraph{Builder.} Küszöbök (\texttt{BREAKER\_MAX\_TOTAL}, \texttt{BREAKER\_MIN\_TOTAL}, 
\texttt{ALLOC\_MAX\_TOTAL}) és vezérlési opciók mellett három szerkesztőablak az 
\texttt{esp1/2/3} menetrendekhez. Menetrend-generátor 
(állandó, rámpa, lépcső, szinusz, random walk) \emph{Insert/Overwrite} móddal. Két fő akció:
\begin{compactitem}
...
\end{compactitem}
Export: \emph{Export JSON} (letöltés) és \emph{Save JSON on server}.
...
\subsection{Megbízhatósági és üzemeltetési szempontok}
\begin{itemize}
  \item \textbf{Atomikus fájlcsere}: ideiglenes állományra írás, majd \emph{rename} 
  minimalizálja a félbemaradt módosítások kockázatát.
  \item \textbf{Determinált sorrend}: mindig \texttt{STOPPED} állapotban történik a 
  konfigurációváltás; a végállapot explicit.
  \item \textbf{Idempotens futtatás}: ugyanazon scenarió többszöri lefuttatása azonos 
  eredményre vezet, ha az input változatlan.
  \item \textbf{Naplókezelés}: \texttt{output.txt} opcionális törlése bemutató előtt; 
  a vezérlő a fejlécet újra létrehozza.
\end{itemize}

\subsection{Használati munkafolyamat}
Bemutatóhoz javasolt lépések:
\begin{enumerate}
  \item \emph{Presets}: kiválasztás \(\rightarrow\) opciók 
  (\emph{Reset/Clear/Start}) \(\rightarrow\) \emph{Run preset}.
  \item \emph{Builder}: finomhangolás, egyedi menetrend generálása/szerkesztése 
  \(\rightarrow\) \emph{Run now}.
  \item \emph{Export/Save}: scenarió JSON letöltése vagy mentése a szerverre 
  későbbi reprodukció céljából.
  \item \emph{Files}: \texttt{output.txt} letöltése, log-részletek beillesztése a jegyzőkönyvbe.
\end{enumerate}

\subsection{Korlátok és jövőbeli bővítések}
\begin{itemize}
  \item Validációk bővítése (pl. negatív időpontok/áramok tiltása, maximum hossz ellenőrzése).
  \item Verziózott scenarió-séma (\texttt{schema\_version}) és kompatibilitási ellenőrzés.
  \item \emph{Batch} futtató: több scenarió egymás utáni automatikus lefuttatása időzítve.
  \item Jogosultság és hitelesítés a Dev Panel író végpontjain (ha éles közegben használjuk).
\end{itemize}

\subsection*{Összegzés}
Az új Dev Panel funkcionalitás egyetlen, áttekinthető kártyában egyesíti a 
demonstrációkhoz szükséges eszközöket: előre beállított demók, testre szabható scenariók, 
azonnali futtatás, állománykezelés és JSON-alapú reprodukció. Ez csökkenti a kézi lépések 
számát, gyorsítja a tesztelést, és megkönnyíti a bemutatók egységes, megismételhető lefolytatását.
\end{comment}