...
Ebben a fejezetben a megtervezett rendszerarchitektúra gyakorlati megvalósítását ismertetem. 
Részletesebben megmutatom a hardveres és szoftveres komponenseket, 
a kommunikációs protokollok implementációját, valamint az adatgyűjtő és megjelenítő rétegeket. 
A fejezet a fizikai rétegtől a "végpontoktól" halad a magasabb szintű szoftveres rétegekig 
(adatbázis, vizualizáció).

\section{Végpontok}

A rendszerben a fizikai réteget a végpontok alkotják, amik közvetlen kapcsolatban állnak a villamos hálózattal. 
Feladatuk a kommunikáció biztosítása: egyrészt a mérési adatok (áramfelvétel, státusz) 
továbbítása a vezérlő felé, másrészt a beavatkozó parancsok végrehajtása a fizikai eszközökön. 
...
\paragraph{Mérési adatok elküldése}
...
Amikor például a központi vezérlő lekéri a \texttt{/metrics} végpontot, a HTTPS-kiszolgáló 
...
történik a Python központi vezérlőhez:
...
A metrika mezők használatával a közpoonti vezérlő fordítás nélkül le tudja képezni a 
bejövő JSON-t a Prometheus-nak megfelelő formátumra 
(breaker\_state és breaker\_command).

\section{Központi vezérlő}

\subsection{Szerepe és technológiai háttere}

A Központi vezérlő a keretrendszer központi döntéshozó egysége.
Míg az ESP végpontok feladata a mérés és a fizikai beavatkozás,
addig a szerver felelős az adatkezelésért,
a korlátok kezeléséért
és az erőforrás-allokációs algoritmus futtatásáért.

A megvalósításhoz a \textbf{Python} nyelvet
és a \textbf{Flask} keretrendszert választottam.
Azért esett erre a választásom, mert
sok előre elkészített könyvtár érhető el hozzá,
és így nem nekem kelett megírnom az egész REST API-t a semmiből.
Ez, pedig így könnyen integrálható
a konténerizált (Docker/Kubernetes) környezetbe.

\subsection{Működési logika}

A szerver működése eseményvezérelt,
a  HTTPS kérések indítják el a vezérlési ciklust.
A folyamat a következő lépésekből áll:

\begin{enumerate}
    \item Adatfogadás
    \item Globális paraméterek olvasása
    \item Allokáció számítása
    \item Válasz és vezérlés
\end{enumerate}
...
A központi vezérlő-ből könnyű továbbítani az adatokat. A megközelítés amit én használtam hogy egy HTTPS /metrics 
...
\begin{lstlisting}[language=Python, caption={Prometheus metrika deklarálása a központi vezérlőn}]
...
A Prometheus adatgyűjtési modellje alapvetően a „pull” elvre épül,
az adatbázis előre megadott időközönként lekérdezi a konfigurált végpontokat.
Alap estben minden ESP8266 IP-címét
külön célpontként kellene felvenni a \texttt{prometheus.yml} konfigurációs fájlba.

Ez a megoldás azonban rugalmatlan,
mivel a Prometheus konfigurációja statikus:
ha új eszközt szeretnénk felvenni ez
a konfigurációs fájl szerkesztését és a Prometheus szolgáltatás
újraindítását igényli.

Ennek kiküszöbölésére a Központi vezérlőt egy dinamikus aggregációs rétegként alkalmazom.
A Prometheus konfigurációjában így egyetlen stabil, statikus célpont szerepel:
maga a Központi vezérlő.

A szerver belső memóriában tartja nyilván a végpontokat és azok legfrissebb méréseit.
Amikor a Prometheus lekérdezi a szerver \texttt{/metrics} végpontját,
a Flask alkalmazás dinamikusan generálja le a választ
az összes jelenleg csatlakoztatott eszköz adataiból.
...
\begin{lstlisting}[caption={Prometheus számára formázott szöveges kimenet példa}]
    {
...
\begin{lstlisting}[language=C++, caption={Válaszadás a /metrics végponton (ESP8266)}]
    sendMetricsToEndpoint();
    // ... metrikak osszeallitasa ...
    server.send(200, "text/plain", metrics);
\end{lstlisting}

\subsection{Prometheus lekérdezések kezelése}

\begin{lstlisting}[language=C++, caption={Adatgyűjtő függvény hívása}]
    queryPrometheus()
\end{lstlisting}
...
\begin{lstlisting}[caption={Prometheus lekérdezési URL formátuma}]
...
\begin{lstlisting}[language=C++, caption={HTTP válasz fogadása és JSON deserializáció}]
...
\begin{lstlisting}[language=C++, caption={JSON válasz feldolgozása és az érték kinyerése}]
    const char* status = doc["status"];
    if (String(status) == "success") {
      
      // Az ertek a data -> result -> value tombben talalhato
...
\subsection{Az áramok vizualizálása és riasztások a Grafanában}
...
\begin{lstlisting}[caption={Példa az exportált metrikák címkézésére}]
...
\begin{lstlisting}[caption={Lekérdezés az EV töltők áramfelvételére}]
...
lehet látni igazából továbbra is a fenti lekérdezésseket használva, 
...
\begin{lstlisting}[caption={Maximum kiválasztása időablakban (segédberendezések)}]
...
\begin{lstlisting}[caption={Napelem bemeneti áram lekérdezése}]
